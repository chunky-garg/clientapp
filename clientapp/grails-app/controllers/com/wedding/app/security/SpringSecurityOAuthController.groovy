...
class SpringSecurityOAuthController {
    ...
    def onSuccess = {
        ...
        if (oAuthToken.principal instanceof GrailsUser) {
            println(" is grails user")
            authenticateAndRedirect(oAuthToken, defaultTargetUrl)
            println(" is grails user autnectivated")
            redirect(controller: 'wedding', action: 'index')
        } else {
            ...
            redirectUrl = "http://local.wedmered.com:8080/clientapp/oauth/askToLinkOrCreateAccount"
